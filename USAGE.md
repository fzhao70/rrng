...
> **v0 covers `sample(..., replace = TRUE)` only** — not sampling without replacement and not
> weighted (`prob=`) sampling. See [scope](#what-is-and-isnt-supported).
...
| `runif(1)` | `g.unif_rand()` |
| `runif(n)` | `g.runif(n)` |
| `sample(1:n, k, replace=TRUE)` | `g.sample_index(n, k) + 1` |
...
- **`replace = FALSE` / weighted sampling.** Not supported in v0; results will *not* match R.
...
`sample(..., replace = TRUE)` under both Rejection (R ≥ 3.6) and Rounding (R < 3.6).

**Not yet supported:** `rnorm`/`rexp`/`rbinom`/`rpois`/`rgamma`; `sample` without replacement;
weighted `sample(prob=)`; non-default RNG kinds and `normal.kind`s. If your script uses these, the
